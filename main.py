...
async def search(
    ...
    try:
        if generate_answer:
            # Use new generate_answer method
            result = get_rag_service().generate_answer(query, n_results, filter_metadata, use_llm=True)
            return {
                "query": query,
                "results_count": result.get('results_count', 0),
                "results": result.get('results', []),
                "answer": result.get('answer'),
                "context_sources": result.get('context_used', 0),
                "context": result.get('context', ''),  # Include context for agent analysis
